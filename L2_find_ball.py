...
def findBall():
    ...
    tc = 50     # Too Close     - Maximum pixel size of object to track
    tf = 6      # Too Far       - Minimum pixel size of object to track
    tp = 40     # Target Pixels - Target size of object to track
    ...
    duty_l = 0.5 # initialize motor with zero duty cycle
    duty_r = -0.5 # initialize motor with zero duty cycle
    ...
    try:
        ...
        while rcpy.get_state() != rcpy.EXITING:
            ...
            if rcpy.get_state() == rcpy.RUNNING:
                ...
                scale_t = 2	# a scaling factor for speeds
                scale_d = 2	# a scaling factor for speeds
                ...
                motor.set(motor_l, duty_l)
                motor.set(motor_r, duty_r)  
                if (0 <= duty_r <= 0.28 and 0 <= duty_l <= 0.28):
                    scan = lidar.polarScan(170)
                    rScan = [[a[0], a[1]] for a in scan if a[1] >= -15 and a[1] <= 15]
                    scanXY = [[a[0]*cos(a[1]*(pi/180)), a[0]*sin(a[1]*(pi/180))] for a in rScan]
                    scanXY = [[round(a[0], 4), round(a[1], 4)] for a in scanXY]
                    txt = open("/home/debian/basics/plot.csv", 'w+')
                    for a in range(len(scanXY)):
                        txt.write(str(scanXY[a][0]) + ", " + str(scanXY[a][1]) + "\n")
                    txt.close()
                    x = [a[0] for a in scanXY]
                    y = [a[1] for a in scanXY]
                    possibleCurvePts = []
                    center = [0, 0]
                    dToCenter = 0
                    cError = 1
                    D = 0
                    possibleCurvePts = [scanXY[a] for a in range(len(scanXY)) if 0.5 <= rScan[a][0] <= 0.6]
                    if len(possibleCurvePts) >= 4:
                        theta1 = atan((possibleCurvePts[1][1] - possibleCurvePts[0][1])/(possibleCurvePts[1][0] - possibleCurvePts[0][0]))
                        theta2 = atan((possibleCurvePts[-1][1] - possibleCurvePts[-2][1])/(possibleCurvePts[-1][0] - possibleCurvePts[-2][0]))
                        D = (sqrt((possibleCurvePts[-2][1] - possibleCurvePts[0][1])**2 + (possibleCurvePts[-2][0] - possibleCurvePts[0][0])**2)) / (2*sin(abs(theta2 - theta1)/2))
                        if theta1 < 0:
                            theta1 += 2*pi
                        if theta2 < 0:
                            theta2 += 2*pi
                        ...
                        if theta1 > theta2:
                            thetaCenter = theta1 - pi/2
                        else:
                            thetaCenter = theta1 + pi/2
                        dx = D * cos(thetaCenter)
                        dy = D * sin(thetaCenter)
                        center = [(possibleCurvePts[0][0] + dx), (possibleCurvePts[0][1] + dy)]
                    dToCenter = sqrt(center[1]**2 + center[0]**2)
                    print("center:", center, "dist to center:", dToCenter, "num points:", len(possibleCurvePts))
                    #print("XY:", scanXY)
                    print("polar:", rScan)
                    #print("circle points:", circlePoints)
                    return(center, dToCenter)
                    break
